...
create-deps: check-existing-riglet
	...
	@read -p 'DB Name (AwsRigDb): (<ENTER> will keep existing) ' DB_NAME; \
			[ -z $$DB_NAME ] || aws ssm put-parameter --region ${REGION} --name "/${OWNER}/${PROJECT}/db/DB_NAME" --description "DB Name" --type "String" --value "$$DB_NAME" --overwrite			
