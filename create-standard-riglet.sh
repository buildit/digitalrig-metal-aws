...
then
  ...
  # echo "Y" | make create-build REPO=bookit-api REPO_BRANCH=${BRANCH} CONTAINER_PORT=8080 CONTAINER_MEMORY=512 LISTENER_RULE_PRIORITY=300 && \
  # echo "Y" | make create-build REPO=bookit-client-react REPO_BRANCH=${BRANCH} SUBDOMAIN=bookit CONTAINER_PORT=4200 CONTAINER_MEMORY=128 LISTENER_RULE_PRIORITY=200
fi
